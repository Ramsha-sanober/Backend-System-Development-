...
app.post('/payment', (req, res) => {
  ...
  db.get(
    "SELECT total_amount, amount_paid, emi FROM loans WHERE loan_id = ?",
    [loan_id],
    (err, loan) => {
      if (err) return res.status(500).json({ error: err.message })
      if (!loan) return res.status(404).json({ error: "Loan not found" })
      let new_amount_paid = loan.amount_paid + payment
      let adjusted_payment = payment
      if (new_amount_paid > loan.total_amount) {
        adjusted_payment = loan.total_amount - loan.amount_paid
        new_amount_paid = loan.total_amount
      }
      db.run(
        "INSERT INTO payments (loan_id, amount) VALUES (?, ?)",
        [loan_id, adjusted_payment],
        err => {
          if (err) return res.status(500).json({ error: err.message })
          db.run(
            "UPDATE loans SET amount_paid = ? WHERE loan_id = ?",
            [new_amount_paid, loan_id],
            err => {
              if (err) return res.status(500).json({ error: err.message })
              const remaining_amount = loan.total_amount - new_amount_paid
              const emi_left = Math.ceil(remaining_amount / loan.emi) || 0
              res.json({
                payment_received: Number(adjusted_payment.toFixed(2)),
...
app.get('/ledger/:loan_id', (req, res) => {
  ...
  db.get(
    "SELECT principal, interest, total_amount, emi, amount_paid FROM loans WHERE loan_id = ?",
    [loan_id],
    (err, loan) => {
      if (err) return res.status(500).json({ error: err.message })
      if (!loan) return res.status(404).json({ error: "Loan not found" })
      db.all(
        "SELECT amount, timestamp FROM payments WHERE loan_id = ? ORDER BY timestamp",
        [loan_id],
        (err, payments) => {
          if (err) return res.status(500).json({ error: err.message })
...
app.get('/account/:customer_id', (req, res) => {
  ...
  db.all(
    "SELECT loan_id, principal, interest, total_amount, emi, amount_paid, period_years, rate FROM loans WHERE customer_id = ?",
    [customer_id],
    (err, loans) => {
      if (err) return res.status(500).json({ error: err.message })
